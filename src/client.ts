...
import { BalanceSheetEntry, FinancialDataEntry, Quote, QuoteListParams, QuoteListResponse, QuoteRetrieveParams, QuoteRetrieveResponse } from './resources/quote';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BRAPI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BRAPI_LOG'] or 'warn' if it isn't set.
   */
...
export class Brapi {
   ...
   */
  constructor({
    baseURL = readEnv('BRAPI_BASE_URL'),
    apiKey = readEnv('BRAPI_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BrapiError(
        'The BRAPI_API_KEY environment variable is missing or empty; either provide it, or instantiate the Brapi client with an apiKey option, like new Brapi({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('BRAPI_LOG'), 'process.env[\'BRAPI_LOG\']', this) ?? defaultLogLevel;
  ...
  static BrapiError = Errors.BrapiError;
...
Brapi.Quote = Quote;
Brapi.Available = Available;
Brapi.V2 = V2;

export declare namespace Brapi {
